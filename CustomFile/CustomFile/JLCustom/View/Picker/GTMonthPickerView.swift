...
//    ]
    ...
    private func initView() {
        ...
        alertContainer.frame = CGRect(x: 0, y: Screen.height, width: Screen.width, height: 250)
        alertContainer.backgroundColor = UIColor.white
        self.addSubview(alertContainer)
        
        let button = UIButton.init()
        button.frame = CGRect(x: Screen.width - 60, y: 0, width: 60, height: 40)
        button.setTitle("确定", for: .normal)
        button.setTitleColor(UIColor.important1, for: .normal)
        button.setTitleColor(UIColor.important2, for: .highlighted)
        button.addTarget(self, action: #selector(confirmButtonClick), for: .touchUpInside)
        alertContainer.addSubview(button)
        
        self.leftPickerView.frame = CGRect(x: 0, y: 40, width: Screen.width / 2.0, height: 210)
        self.leftPickerView.delegate = self
        self.leftPickerView.dataSource = self
        alertContainer.addSubview(self.leftPickerView)
        
        self.rightPickerView.frame = CGRect(x: Screen.width / 2.0, y: 40, width: Screen.width / 2.0, height: 210)
    ...
    func show() {
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.alertContainer.y = Screen.height - 250
        }
    }
    ...
    @objc func dismiss() {
        UIView.animate(withDuration: 0.3, animations: {
            self.bgMask.alpha = 0
            self.alertContainer.y = Screen.height
        }) { (finished) in
