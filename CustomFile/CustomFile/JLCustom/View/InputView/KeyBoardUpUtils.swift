...
final class KeyBoardViewUpUtils: NSObject {
    ...
    private func initKeyBoardObserVer() {
        ...
        (NotificationCenter.default.rx.notification(Notification.Name.UIKeyboardWillChangeFrame, object: nil)).subscribe {[weak self] (sender) in
    ...
    private func handlerKeyboardNotice(sender:Notification) {
        ...
        let animation:TimeInterval = userinfo[UIKeyboardAnimationDurationUserInfoKey] as! TimeInterval
...
class KeyBoardUtils: NSObject {
    ...
    func initKeyBoardObserVer(view:UIView) {
        ...
        ///监听键盘显示 did
        (NotificationCenter.default.rx.notification(Notification.Name.UIKeyboardDidShow, object: nil)).subscribe {[weak self] (sender) in
            if sender.element != nil {
                self?.KEYBOARDDIDAPPEAR = true
            }
            }.disposed(by:keyboardBg)
        let _ = (NotificationCenter.default.rx.notification(Notification.Name.UIKeyboardDidHide, object: nil)).subscribe {[weak self] (sender) in
            if sender.element != nil {
        ...
        ///监听键盘显示 will
        (NotificationCenter.default.rx.notification(Notification.Name.UIKeyboardWillShow, object: nil)).subscribe {[weak self] (sender) in
            if sender.element != nil {
                self?.KEYBOARDWILLAPPEAR = true
            }
            }.disposed(by:keyboardBg)
        let _ = (NotificationCenter.default.rx.notification(Notification.Name.UIKeyboardWillHide, object: nil)).subscribe {[weak self] (sender) in
            if sender.element != nil {
