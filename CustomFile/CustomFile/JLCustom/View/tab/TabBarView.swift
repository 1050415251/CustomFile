...
class TabBarView: UIScrollView {
    ...
    ///选中item字体大小
    private var SELECTED_FONT = UIFont.font(ofSize: 16)
    ///未选中item字体大小
    private var UN_SELECTED_FONT = UIFont.font(ofSize: 16)
    ///选中item下划线颜色
    ...
    func setItemsPropretry(selecttextC:UIColor,unselecttextC:UIColor,selectlineColor:UIColor,selectfontSize:CGFloat,unselectfontSize:CGFloat,backGroundc:UIColor,lineWidth:CGFloat? = nil) {
        ...
        SELECTED_FONT = UIFont.font(ofSize: selectfontSize)
        UN_SELECTED_FONT = UIFont.font(ofSize: unselectfontSize)
    ...
    func setTipsNewsView(news:String,selecttextC:UIColor,unselecttextC:UIColor,fontSize:CGFloat,backGroundc:UIColor,index:Int,frame:CGRect) {
        ...
        tipsNews[index].font = UIFont.font(ofSize: fontSize)
        tipsNews[index].layer.masksToBounds = true
        tipsNews[index].layer.cornerRadius = frame.height * 0.5
        
        let width = (news as NSString).boundingRect(with:  CGSize(width:CGFloat(MAXFLOAT),height: CGFloat(MAXFLOAT)), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font:UIFont.font(ofSize: fontSize)], context: nil).width
