...
class JLTableViewDataSource<T: BaseBean>: NSObject,UITableViewDelegate {
...
//        })
    ...
    func requestToServer(params: RequestParams) -> Observable<[T]> {
        //return JLRxNetRequest<T>.reuqestinfoToServer(requestParams: requestParamswe)
        var ob = Observable<[T]>()
        JLRxNetRequest<T>.reuqestinfoToServer(requestParams: requestParams).subscribe(onNext: { [weak self] data in
             ob.just(data)
