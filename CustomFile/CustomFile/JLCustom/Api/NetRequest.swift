...
//               // p["userId"] = User.shared.usrId ?? ""
//               // p["token"] = User.shared.token ?? ""
//            }
//        }
//
...
//    class func requestList(method:HTTPMethod,url:String,params:[String:Any]?,jsonDatacallback:((JSON)->Void)?,successful:(([T]?) -> Void)?,failed:((NSError)->Void)?,dataKey:String?) {
//        let byForm = url.hasPrefix("if") || url.hasPrefix("activityIf")
//        var headers:[String:String] = ["Content-Type":byForm ? "application/x-www-form-urlencoded":"application/json"]
//        if User.shared.isLogined {
//            headers.updateValue(User.shared.token ?? "", forKey: "token")
//            headers.updateValue(User.shared.usrId ?? "", forKey: "userId")
//        }
...
//        for item in imgs {
//            imageDataArray.append(zipPicture(item: item, imgData: UIImageJPEGRepresentation(item, 1.0)!).base64EncodedData())
//
//        }
//        if !isasync {
//            HUDUtil.showHud()
...
//                }
//            }
//            if let userId = User.shared.usrId {
//                let data = userId.data(using: String.Encoding.utf8)!
//                multipartFormData.append(data, withName: "userId")
//            }
//
...
//
//            compression-=0.1
//            newimgData = UIImageJPEGRepresentation(item, compression)!
//        }
